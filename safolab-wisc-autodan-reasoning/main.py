...
from llm import HuggingFaceModel, OpenAIEmbeddingModel, VLLMModel
...
if __name__ == "__main__":
    ...
    if args.hot_lifelong:
        with open("./logs/warm_up_strategy_library.pkl", "rb") as f:
            if os.environ.get("ALLOW_UNTRUSTED_PICKLE") == "1":
                warm_up_strategy_library = pickle.load(f)  # noqa: S301 - guarded by ALLOW_UNTRUSTED_PICKLE
            else:
                raise RuntimeError(
                    "Deserializing pickle files is disabled by default. To enable, set ALLOW_UNTRUSTED_PICKLE=1"
                )
        with open("./logs/warm_up_attack_log.json", encoding="utf-8") as f:
            warm_up_attack_log = json.load(f)
        with open("./logs/warm_up_summarizer_log.json", encoding="utf-8") as f:
            warm_up_summarizer_log = json.load(f)
        autodan_turbo_pipeline.hot_start_lifelong(warm_up_attack_log)
    else:
        if args.hot:
            with open(f"./logs/warm_up_attack_log{suffix}.json", encoding="utf-8") as f:
                init_attack_log = json.load(f)
            warm_up_strategy_library, warm_up_attack_log, warm_up_summarizer_log = (
                autodan_turbo_pipeline.hot_start(init_attack_log)
            )
        else:
            warm_up_strategy_library, warm_up_attack_log, warm_up_summarizer_log = (
                autodan_turbo_pipeline.warm_up(init_library, init_attack_log, init_summarizer_log)
            )
