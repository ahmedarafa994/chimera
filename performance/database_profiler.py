...
POSTGRES_AVAILABLE = (
    _importlib.find_spec("asyncpg") is not None
    or _importlib.find_spec("psycopg2") is not None
)
...
class DatabaseProfiler:
    ...
    def profile_query(self, query: str, database_type: str = "sqlite",
                     connection_id: str | None = None) -> QueryMetrics:
    ...
    def _execute_sqlite_query(self, query: str) -> tuple:
        ...
        try:
            ...
            execution_plan = {
                "plan_rows": [{"id": row[0], "parent": row[1], "detail": row[3]} for row in plan_rows]
            }
    ...
    def _analyze_query_indexes(self, query: str) -> list[str]:
        ...
        where_match = re.search(r'\bwhere\s+(.+?)(?:\border\s+by|\bgroup\s+by|\blimit\s+|\bhaving\s+|$)',
    ...
    def profile_cache_operation(self, operation: str, key: str,
                              data: Any = None, ttl: int | None = None) -> CacheMetrics:
    ...
    def generate_optimization_recommendations(self) -> list[str]:
        ...
        if slow_queries:
            ...
            for query in slow_queries[:10]:  # Top 10 slow queries
                if not query.indexes_used:
                    recommendations.append(f"Consider adding indexes for query: {query.query[:100]}...")
        ...
        if cache_stats:
            ...
            if overall_hit_rate < 0.8:  # Less than 80% hit rate
                recommendations.append(f"Cache hit rate is {overall_hit_rate:.1%} - consider optimizing cache strategy")

            if overall_hit_rate < 0.5:  # Less than 50% hit rate
                recommendations.append("Very low cache hit rate - review caching patterns and TTL values")
        ...
        if self.connection_metrics:
            ...
            for metric in recent_pool_metrics:
                utilization = metric.active_connections / metric.max_connections if metric.max_connections > 0 else 0

                if utilization > 0.8:  # Over 80% pool utilization
                    recommendations.append(f"High connection pool utilization ({utilization:.1%}) for {metric.pool_name}")

                if metric.queue_size > 0:
                    recommendations.append(f"Connection pool queue detected for {metric.pool_name} - consider increasing pool size")
    ...
    def generate_database_report(self) -> DatabasePerformanceReport:
        ...
        performance_summary = {
            ...
            "avg_query_time_ms": statistics.mean([q.execution_time_ms for q in self.query_metrics]) if self.query_metrics else 0,
        ...
        if slow_queries:
            performance_summary["slowest_query_time_ms"] = max(q.execution_time_ms for q in slow_queries)
