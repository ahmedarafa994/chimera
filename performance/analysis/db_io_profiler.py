...
class DatabaseProfiler:
    ...
    async def connection_pool_analysis(self) -> dict[str, Any]:
        ...
        pool_stats = {
            ...
            'connections_in_use': self.connection_pool.get_size() - len(self.connection_pool._holders)
        }
...
def generate_performance_charts(report: dict[str, Any]):
    ...
    try:
        ...
        io_data = pd.DataFrame([
            {'operation': 'Read', 'throughput': report['io']['benchmark']['read_throughput_mbps']},
            {'operation': 'Write', 'throughput': report['io']['benchmark']['write_throughput_mbps']}
        ])
        ...
        plt.tight_layout()
        plt.savefig(f'performance/analysis/performance_charts_{int(time.time())}.png', dpi=300, bbox_inches='tight')
        plt.close()
