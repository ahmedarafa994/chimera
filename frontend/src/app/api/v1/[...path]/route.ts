...
// AutoDAN reasoning, GPTFuzz, and other AI operations can take several minutes
export const maxDuration = 600; // 10 minutes in seconds (Vercel/Next.js config)
...
async function proxyToBackend(
  ...
  const upstreamUrl = new URL(`${backend.apiBaseUrl}/${params.path.join("/")}`);
  ...
  try {
    ...
    if (error instanceof Error && (
      ...
      console.error("[API Proxy] Timeout error:", error.message);
      return NextResponse.json(
        {
