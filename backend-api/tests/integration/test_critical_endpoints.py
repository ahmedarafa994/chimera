...
class TestCriticalEndpoints:
    ...
    def test_providers_endpoint_integration(self, authenticated_client):
        """Test providers endpoint with authentication"""
        response = authenticated_client.get("/api/v1/providers")
        assert response.status_code in [200, 401]

    def test_session_models_endpoint(self, authenticated_client):
        """Test session models endpoint"""
        response = authenticated_client.get("/api/v1/session/models")
        assert response.status_code in [200, 401]

    def test_transform_endpoint_validation(self, authenticated_client):
    ...
    def test_jailbreak_generate_endpoint(self, authenticated_client):
