...
@router.get("/{dataset_name}/prompts")
async def get_prompts(
    dataset_name: str,
    risk_area: Annotated[
        str | None, Query(description="Filter by risk area (e.g., 'I', 'II', 'III')")
    ] = None,
    harm_type: Annotated[
        str | None, Query(description="Filter by harm type (e.g., 'I-1', 'II-2')")
    ] = None,
    severity: Annotated[
        str | None, Query(description="Filter by severity (low, medium, high, critical)")
    ] = None,
    limit: Annotated[int, Query(ge=1, le=1000, description="Maximum prompts to return")] = 100,
    shuffle: Annotated[bool, Query(description="Randomize the results")] = False,
...
@router.get("/{dataset_name}/random")
async def get_random_prompts(
    dataset_name: str,
    count: Annotated[int, Query(ge=1, le=100, description="Number of random prompts")] = 10,
    risk_area: Annotated[str | None, Query(description="Filter by risk area")] = None,
...
@router.get("/{dataset_name}/taxonomy")
...
@router.get("/{dataset_name}/risk-areas")
...
@router.get("/{dataset_name}/statistics")
...
@router.post("/{dataset_name}/test-batch")
async def generate_test_batch(
    dataset_name: str,
    batch_size: Annotated[int, Query(ge=1, le=100, description="Number of prompts")] = 10,
    balanced: Annotated[bool, Query(description="Balance across risk areas")] = True,
    risk_areas: Annotated[list[str] | None, Query(description="Risk areas to include")] = None,
    severities: Annotated[list[str] | None, Query(description="Severities to include")] = None,
) -> list[BenchmarkPrompt]:
...
async def export_for_jailbreak(
    dataset_name: str,
    risk_area: Annotated[str | None, Query(description="Filter by risk area")] = None,
    limit: Annotated[int | None, Query(ge=1, le=1000, description="Maximum prompts")] = None,
) -> list[dict[str, Any]]:
