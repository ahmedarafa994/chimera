...
Provider Details:
- API: https://api.cursor.sh/v1/chat/completions
- Authentication: Bearer token (API key)
- Default Models: cursor-small, cursor-large, cursor-tiny
...
class CursorProvider(BaseProvider):
    ...
    # Cursor API endpoints (OpenAI-compatible)
    _BASE_URL = "https://api.cursor.sh/v1"
    ...
    async def _generate_impl(
        ...
        try:
            ...
            logger.error(f"[cursor] HTTP error: {e.response.status_code} - {e.response.text}")
    ...
    async def _check_health_impl(self, client: httpx.AsyncClient) -> bool:
        ...
        try:
            ...
            logger.debug(f"[cursor] Health check failed: {e}")
            return False
            ...
            async with client.stream("POST", "chat/completions", json=payload) as response:
                ...
                async for line in response.aiter_lines():
                    ...
                    try:
                        ...
                        logger.warning(f"[cursor] Failed to parse streaming chunk: {data_str}")
                        continue
