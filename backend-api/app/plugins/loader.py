    ...
    def clear(self) -> None:
        ...
        for plugin in self._plugins.values():
            try:
                ...
                logger.exception(f"Error cleaning up plugin {plugin.metadata.name}: {e}")
        self._plugins.clear()
...
class PluginLoader:
    ...
    def __init__(
        self, plugin_dir: Path | None = None, registry: PluginRegistry | None = None
    ) -> None:
    ...
    def load_plugin(self, file_path: str) -> int:
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if self.registry.register(attr):
                    try:
                        ...
                        logger.exception(f"Plugin initialization failed: {attr_name}: {e}")
            ...
            logger.exception(f"Failed to load plugin file {path}: {e}")
            return 0
