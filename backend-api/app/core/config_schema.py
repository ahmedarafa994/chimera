...
from app.domain.provider_models import ProviderType
...
class ProxyModeConfig(BaseModel):
    ...
    def validate_endpoint(cls, v):
        ...
        if not v.startswith(("http://", "https://")):
            msg = "Proxy endpoint must be a valid URL"
            raise ValueError(msg)
        return v.rstrip("/")
...
class ChimeraConfiguration(BaseModel):
    ...
    validate_connectivity: bool = Field(
        default=True,
        description="Test provider connectivity during validation",
    )
    ...
    log_api_keys: bool = Field(
        default=False,
        description="Log API keys (NEVER enable in production)",
    )
