...
class DatabaseQueryOptimizer:
    ...
    async def analyze_and_optimize(self) -> dict[str, Any]:
        ...
        n_plus_one_analysis = await self._analyze_n_plus_one_patterns(performance_report['n_plus_one_patterns'])
        ...
        return {
            ...
            "analysis_summary": {
                "total_recommendations": len(self.index_recommendations) + len(self.query_optimizations),
                "high_priority_items": len([r for r in self.index_recommendations if r.estimated_improvement == "HIGH"]) +
            ...
            "index_recommendations": [self._index_to_dict(idx) for idx in self.index_recommendations],
            "query_optimizations": [self._query_opt_to_dict(opt) for opt in self.query_optimizations],
    ...
    async def _analyze_table_indexes(self, session: AsyncSession, table_name: str):
        ...
        table_queries = [
            (hash_, metrics) for hash_, metrics in db_profiler.query_metrics.items()
        ...
        for _query_hash, metrics in table_queries:
            ...
            if metrics.execution_count > 50:
                ...
                if select_columns and where_columns:
                    covering_columns = where_columns + [col for col in select_columns if col not in where_columns]
                    self.index_recommendations.append(IndexRecommendation(
                        table_name=table_name,
    ...
    async def _analyze_slow_queries(self, slow_queries: list[dict]) -> list[QueryOptimizationSuggestion]:
    ...
    def _generate_query_optimizations(self, query: str, avg_time_ms: float) -> list[QueryOptimizationSuggestion]:
    ...
    def _generate_n_plus_one_resolutions(self, query: str, occurrences: int) -> list[dict[str, Any]]:
    ...
    async def _analyze_connection_pools(self, pool_data: list[dict]) -> dict[str, Any]:
        ...
        return {
            "total_pools_analyzed": len(pool_data),
    ...
    def _calculate_overall_improvement(self) -> str:
        ...
        high_impact_count = len([r for r in self.index_recommendations if r.estimated_improvement == "HIGH"])
        high_impact_count += len([q for q in self.query_optimizations if q.estimated_improvement == "HIGH"])
