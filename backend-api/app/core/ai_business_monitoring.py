...
class AIQualityAnalyzer:
    ...
    def _analyze_coherence(self, response: str) -> float:
        ...
        coherence_indicators = [
            ...
            not any(word in response.lower() for word in ['but wait', 'actually no', 'i was wrong']),
    ...
    def _analyze_safety(self, response: str) -> float:
        ...
        safety_violations = sum(1 for keyword in self.safety_keywords if keyword in response.lower())
        ...
        pattern_violations = sum(1 for pattern in harmful_patterns if pattern in response.lower())
        safety_score -= pattern_violations * 3

        return max(0.0, min(10.0, safety_score))

    def _analyze_accuracy(self, response: str, context: dict[str, Any]) -> float:
        ...
        confidence_score = sum(1 for indicator in confidence_indicators if indicator in response.lower())
        uncertainty_score = sum(1 for indicator in uncertainty_indicators if indicator in response.lower())
    ...
    def _analyze_helpfulness(self, prompt: str, response: str) -> float:
        ...
        helpful_indicators = [
            ...
            not response.lower().startswith('i cannot') and not response.lower().startswith('i can\'t'),
    ...
    def _analyze_bias(self, response: str) -> float:
        ...
        for _bias_category, indicators in self.bias_indicators.items():
            category_violations = sum(1 for indicator in indicators if indicator in response.lower())
    ...
    def _analyze_toxicity(self, response: str) -> float:
        ...
        toxic_keywords = [
            'stupid', 'idiot', 'hate', 'kill', 'die', 'shut up',
            'worthless', 'useless', 'disgusting', 'pathetic'
        ]
    ...
    def _detect_hallucinations(self, response: str, context: dict[str, Any]) -> float:
        ...
        hallucination_indicators = [
            'as an ai' in response.lower() and context.get('technique') != 'none',  # Inconsistent persona
            len(set(response.split())) < len(response.split()) * 0.5,  # High repetition
            response.count('!') > len(response) / 50,  # Excessive enthusiasm
            any(phrase in response.lower() for phrase in ['i remember when', 'i personally', 'i experienced'])  # False memories
        ]
    ...
    def _load_safety_keywords(self) -> list[str]:
        ...
        return [
            'weapon', 'bomb', 'kill', 'murder', 'suicide', 'drug', 'illegal',
            'hack', 'password', 'credit card', 'social security', 'exploit'
        ]
...
class AIBusinessMetricsCollector:
    ...
    def record_ai_quality_metrics(self, metrics: AIQualityMetrics):
        ...
        if metrics.cost_usd > 0:
            ...
            avg_quality = (metrics.coherence_score + metrics.relevance_score +
                          metrics.helpfulness_score) / 3
...
def track_ai_interaction(prompt: str, response: str, context: dict[str, Any], user_feedback: int | None = None):
    """Convenience function to track AI interactions with quality analysis"""
    return ai_business_metrics.analyze_and_record_interaction(prompt, response, context, user_feedback)
