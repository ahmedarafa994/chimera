        ...
        **kwargs,
        ...
        if isinstance(data_source, pd.DataFrame):
            ...
            for start_idx in range(0, total_rows, chunk_size):
                ...
                # Process chunk
                processed_chunk = await self._process_chunk_safe(chunk, processor_func, **kwargs)
...
class ParallelDataPipelineEngine:
    ...
    def __init__(self, config: OptimizedIngestionConfig):
        ...
        self.process_pool = (
            ProcessPoolExecutor(max_workers=config.process_pool_size)
    ...
    async def _combine_dataframes_optimized(self, dataframes: list[pd.DataFrame]) -> pd.DataFrame:
