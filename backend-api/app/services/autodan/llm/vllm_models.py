...
class VLLMModel:
    def __init__(self, repo_name: str, config_dir: str, config_name: str, token=None):
        ...
        """
        if LLM is None:
            raise ImportError("vllm is not installed or not supported on this platform.")
        ...
        self.llm = LLM(
            model=model_path, trust_remote_code=True, tensor_parallel_size=torch.cuda.device_count()
        )
    ...
    def continue_generate(
        self, system: str, user1: str, assistant1: str, user2: str, max_length: int = 1000, **kwargs
    ):
        """
    ...
    def conditional_generate(
        self, condition: str, system: str, user: str, max_length: int = 1000, **kwargs
    ):
        """
