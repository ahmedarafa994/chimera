...
class CausalConfig(BaseModel):
    ...
    exploration_budget: int = Field(
        ...
        ge=100,
        le=100000,
        description="Number of explorations per session",
    )
    asymmetry_threshold: float = Field(
        2.0,
        ge=1.0,
        le=10.0,
        description="Threshold for considering a relationship asymmetric",
    )
    hidden_variable_penalty: float = Field(
        0.2,
        ge=0.0,
        le=1.0,
        description="Penalty for paths through hidden variables",
    )
...
class EvolutionConfig(BaseModel):
    ...
    mutation_rate: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Probability of heuristic mutation",
    )
    crossover_rate: float = Field(
        0.3,
        ge=0.0,
        le=1.0,
        description="Probability of heuristic crossover",
    )
    selection_pressure: float = Field(
        2.0,
        ge=1.0,
        le=10.0,
        description="Tournament selection pressure",
    )
    elitism_ratio: float = Field(
        0.1,
        ge=0.0,
        le=1.0,
        description="Ratio of elite heuristics preserved",
    )
...
class FeedbackConfig(BaseModel):
    ...
    fast_loop_interval_ms: int = Field(
        ...
        ge=10,
        le=1000,
        description="Fast feedback loop interval in milliseconds",
    )
    medium_loop_interval_sec: int = Field(
        60,
        ge=10,
        le=3600,
        description="Medium feedback loop interval in seconds",
    )
    slow_loop_interval_hours: int = Field(
        24,
        ge=1,
        le=168,
        description="Slow feedback loop interval in hours",
    )
    ...
    adjustment_factor: float = Field(
        ...
        ge=0.01,
        le=0.5,
        description="Factor for heuristic weight adjustment",
    )
...
class HeuristicConfig(BaseModel):
    ...
    novelty_threshold: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Minimum novelty score for new heuristics",
    )
    composition_depth_limit: int = Field(
        5,
        ge=1,
        le=20,
        description="Maximum depth for heuristic composition",
    )
    abstraction_levels: int = Field(
        5,
        ge=1,
        le=10,
        description="Number of abstraction hierarchy levels",
    )
...
class FailureDetectionConfig(BaseModel):
    ...
    sensitivity: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Sensitivity threshold for failure detection",
    )
    exploitability_threshold: float = Field(
        0.5,
        ge=0.0,
        le=1.0,
        description="Threshold for exploitability scoring",
    )
    min_symptoms: int = Field(
        2,
        ge=1,
        le=10,
        description="Minimum symptoms for failure classification",
    )
...
class IntegrationConfig(BaseModel):
    ...
    autodan_weight: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Weight for AutoDAN in hybrid attacks",
    )
    gptfuzz_weight: float = Field(
        0.4,
        ge=0.0,
        le=1.0,
        description="Weight for GPTFuzz in hybrid attacks",
    )
    hybrid_strategy: Literal["sequential", "parallel", "adaptive"] = Field(
        "adaptive",
        description="Strategy for combining AutoDAN and GPTFuzz",
    )
