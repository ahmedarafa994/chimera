...
def log(message, type="info", verbose_level=VERBOSE_NORMAL):
    ...
    if type == "info":
        ...
        prefix = f"{Back.GREEN}{Fore.BLACK}[RESULT]{Style.RESET_ALL}"
    else:
        prefix = f"[{type.upper()}]"
...
def display_config(config_dict, width=80):
    ...
    print(
        f"{Fore.CYAN}ATTACKER MODEL:{Style.RESET_ALL} {config_dict.get('attacker_model', 'gpt-4o-mini')}"
    )
    ...
    print(
        f"  • Verbosity Level: {VERBOSE_LEVEL_NAMES.get(config_dict.get('verbosity_level', VERBOSE_NORMAL), 'Detailed')}"
    )
    ...
    print(f"  • Followup Prompt: {config_dict.get('system_prompt_followup', 'Not Used')}")
