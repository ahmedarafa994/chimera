...
class TokenCalculator:
    ...
    def calculate_tokens(self, text: str, tokenModel=None) -> int:
        ...
        try:
            ...
            if ("gpt" in model.lower() or model in tiktoken_encodings) and TIKTOKEN_AVAILABLE:
                if model not in self._tokenizers:
                    try:
                        ...
                        log(
                            f"Unknown model {model}, falling back to cl100k_base encoding",
                            "warning",
                            VERBOSE_DETAILED,
                        )
            ...
            # Handle Grok models specially, use cl100k_base encoding
                ...
                if model not in self._tokenizers:
                    ...
                    log(
                        f"Using cl100k_base encoding for Grok model {model}",
                        "info",
                        VERBOSE_DETAILED,
                    )
                    self._tokenizers[model] = tiktoken.get_encoding("cl100k_base")
