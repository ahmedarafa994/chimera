...
class AttackerLLM(LLM):
    ...
    def __init__(
        ...
        if "grok" in self.model_key.lower():
            ...
            self.api_type = model_config.get("api", "openai")  # Default to openai if not specified
    ...
    def rewrite(self, prompt):
        ...
        if "grok" in self.model_key.lower():
            ...
            system_prompt = self.initial_instructions  # Make sure this matches your variable name
    ...
    def _generate_with_grok(self, instructions, prompt_or_history):
        ...
        try:
            ...
            if instructions:
                [{"role": "system", "content": instructions}, *formatted_messages]
            else:
                pass
    ...
    def adjust_temperature_smart(self, target_response, strategy="adaptive", original_prompt=None):
        ...
        if use_strongreject and original_prompt:
            try:
                ...
                log(
                    f"StrongREJECT evaluation failed: {e}, using default score",
                    "warning",
                    VERBOSE_DETAILED,
                )
        ...
        log(
            f"Temperature adjusted: {self.temperature:.3f} (success_score: {success_score:.3f}, strategy: {strategy})",
            "debug",
            VERBOSE_DETAILED,
        )
