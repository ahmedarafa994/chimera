...
from .config import SUCCESSFUL_PATTERNS_PATH, VERBOSE_DETAILED
...
class PatternManager:
        ...
        if os.path.exists(self.filepath):
            try:
                ...
                if not self._validate_pattern_data(data):
                    logging_utils_log(
                        f"Invalid pattern data structure in {self.filepath}, using defaults",
                        "warning",
                    )
                    return False
    ...
    def save(self):
        ...
        try:
            ...
            logging_utils_log(
                f"Saving {len(self.patterns['effective_prompts'])} prompts to {self.filepath}",
                "debug",
                VERBOSE_DETAILED,
            )
            ...
            with open(self.filepath) as f:
                ...
                logging_utils_log(
                    f"Verified: {len(saved_data.get('effective_prompts', []))} prompts saved",
                    "debug",
                    VERBOSE_DETAILED,
                )
            ...
            if os.path.exists(f"{self.filepath}.tmp"):
                try:
                    ...
                    logging_utils_log(f"Could not remove temp file: {e}", "warning")
            return False
    ...
    def _update_learning_effectiveness(self, successful_turn_number):
        ...
        if successful_turn_number == 1:
            ...
            learning_metrics["single_turn_success_rate"] = (
                single_turn_count / learning_metrics["total_successes"]
            )
        else:
            multi_turn_count = learning_metrics.get("multi_turn_count", 0) + 1
            learning_metrics["multi_turn_count"] = multi_turn_count
            learning_metrics["multi_turn_success_rate"] = (
                multi_turn_count / learning_metrics["total_successes"]
            )
        ...
        learning_metrics["avg_turns_to_success"] = (
            total_turns_used / learning_metrics["total_successes"]
        )
        ...
        if learning_metrics["total_successes"] > 20:
            ...
            recent_single_rate = (
                learning_metrics.get("single_turn_count", 0) / learning_metrics["total_successes"]
            )
            learning_metrics["learning_acceleration"] = (
                recent_single_rate * 2.0
    ...
    def generate_system_prompt_hints(self, target_model=None, turn_number=None):
        ...
        for prompt_data in self.patterns["effective_prompts"]:
            ...
            for technique in techniques:
                ...
                if turn_number is not None and abs(turn - turn_number) <= 1:
