...
def test_prompt_generation_instructions():
    ...
    print("=" * 60)
    print("TEST 1: Verify prompt generation instructions")
    print("=" * 60)
    ...
    print("\n--- Generated LLM Instruction Prompt ---")
...
def test_creative_prompt_template():
    ...
    print("\n" + "=" * 60)
    print("TEST 2: Verify creative generation prompt")
    print("=" * 60)
...
def test_validation_logic_standalone():
    ...
    print("\n" + "=" * 60)
    print("TEST 3: Test validation logic (standalone)")
    print("=" * 60)
    ...
    for input_text, expected_valid, description in test_cases:
        ...
        print(
            f"         Input: '{input_text[:50]}...' -> Valid: {is_valid} (expected: {expected_valid})"
        )
        if not passed:
            all_passed = False
...
async def test_actual_llm_generation():
    ...
    print("\n" + "=" * 60)
    print("TEST 4: Test actual LLM generation (requires API key)")
    print("=" * 60)
    ...
    if not settings.GOOGLE_API_KEY and not settings.OPENAI_API_KEY:
        print("\n[SKIP]: No API keys configured (GOOGLE_API_KEY or OPENAI_API_KEY)")
        print("  Set API keys in .env to test actual LLM generation")
        return None
    ...
    try:
        ...
        print("\n--- Calling LLM to generate adversarial prompt ---")
        ...
        print(f"\n[ERROR]: {e}")
        print("  (This may be due to API configuration issues, not the code changes)")
        return None
...
async def main():
    """Run all tests."""
    print("\n" + "=" * 60)
    print("AutoDAN AI Generation Verification Tests")
    print("=" * 60)
    ...
    if llm_result is not None:
        ...
        print("\n  [SKIP]: LLM generation test skipped (API issues or no keys)")
    ...
    if all_passed:
        ...
        print("  1. Prompts instruct LLM to GENERATE/CREATE original content")
        print("  2. Templates are used as GUIDANCE only, not fill-in-the-blank")
        print("  3. Validation rejects outputs with unfilled placeholders")
        print("  4. Output must be substantial (>100 chars) and creative")
    else:
